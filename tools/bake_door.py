...
def main():
    ...
    if args.sign:
        ...
        if gx:
            # Polarity: green-dominant art (green plate, white pictogram) IS the
            # sign — crop tight. White-dominant art (green text on a white
            # plate) keeps a margin so the plate shows around the letters.
            bw = max(gx) - min(gx) + 1; bh = max(gy) - min(gy) + 1
            green_frac = len(gx) / float(bw * bh)
            pad = 0 if green_frac > 0.5 else 3
            ...
            dst_h = min(SIGN_BAND - 2, int(round(dst_w / asp)))
            dst_w = int(round(dst_h * asp))
            if dst_w > W - 6:
                dst_w = W - 6; dst_h = int(round(dst_w / asp))
            # BOX = plain area average: each texel is the white-vs-green
            # coverage of its source patch. LANCZOS ringing at this extreme
            # downscale (900px -> ~28) smears a pictogram into blobs.
            sign = sign.resize((dst_w, dst_h), Image.BOX)
            sp = sign.load()
            # Adaptive split: score = greenness; threshold at the midpoint of
            # the sign's own extremes, so a texel goes white only when its
            # patch was majority white. Absolute thresholds (r<60 etc.) tagged
            # every green/white blend as white and dilated the figure shut.
            scores = {}
            for yy in range(dst_h):
                for xx in range(dst_w):
                    r, g, b, a = sp[xx, yy]
                    if a >= 128:
                        scores[(xx, yy)] = g - (r + b) / 2.0
            smin = min(scores.values()); smax = max(scores.values())
            sthr = (smin + smax) / 2.0
            ox = (W - dst_w) // 2
            ...
            for yy in range(dst_h):
                for xx in range(dst_w):
                    ...
                    tx = ox + (dst_w - 1 - xx)
                    ty = oy + yy
                    if scores[(xx, yy)] > sthr:
                        v = 6 if (r + g + b) < 220 else 7   # green
                    else:
                        v = 8                               # white
                    rows[ty][tx] = v
    ...
    with open(args.out, "w") as f:
        ...
        f.write(" * Regenerate with: tools/bake_door.py%s%s%s%s%s\n" % (
            " --src %s" % args.src if args.src != "models/render/door_front.png" else "",
            " --flip" if args.flip else "",
            " --grime %d" % args.grime if args.grime else "",
            " --stipple %d" % args.stipple if args.stipple else "",
            " --sign %s" % args.sign if args.sign else ""))
